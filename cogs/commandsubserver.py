...
def hasSubserverRoles():
    ...
    def predicate(ctx):
        return Commandsubserver.utils.hasRoles(
            ctx.author.id, Commandsubserver.subserver_role
        )
...
class Commandsubserver(commands.Cog, name="Subserver Commands"):
    ...
    def __init__(self, bot):
        ...
        Commandsubserver.subserver_role = self.ch.get_subserver_needed_roles()
    ...
    async def create(self, ctx, subserver_name):
        ...
        if "sub-" + subserver_name in guild.roles:
            await ctx.send(
                f"There already exists a subserver with the name {subserver_name}.",
                delete_after=3600,
            )
            return
        ...
        sub_category = await guild.create_category(
            name="sub-" + subserver_name,
            overwrites={
                ...
                guild.default_role: discord.PermissionOverwrite(
                    read_messages=False, connect=False, view_channel=False
                ),
            },
        )

        # Creates subserver channel
        sub_channel = await guild.create_text_channel(
            name="text-" + subserver_name, category=sub_category
        )
        sub_voice_channel = await guild.create_voice_channel(
            name=subserver_name + " #1",
        ...
        sub_way_category = find(
            lambda c: c.name == "Subserver Gateway", guild.categories
        )
        # If no subway category is available, create one
        if not sub_way_category:
            sub_way_category = await guild.create_category(
                name="Subserver Gateway",
                overwrites={
                    guild.me: discord.PermissionOverwrite(
                        manage_channels=True, view_channel=True
                    ),
        ...
        await guild.create_voice_channel(
            name=subserver_name + " (0/0/0)",
            category=sub_way_category,
            overwrites={
                guild.me: discord.PermissionOverwrite(
                    manage_channels=True, view_channel=True
                ),
                sub_way_role: discord.PermissionOverwrite(
    ...
    async def remove(self, ctx, name):
        ...
        sub_way_channel = find(
            lambda ch: ch.name.startswith(name), sub_way_category.voice_channels
        )
        if not (sub_category or sub_way_channel):
            await ctx.send(
                f"ERROR: No subserver with name {name} found.", delete_after=3600
            )
            return
        if sub_category:
    ...
    async def list(self, ctx):
        ...
        for subserver_name in info_dict.keys():
            ...
            embed.add_field(
                name=subserver_name,
                value=f"({user_connected}/{user_online}/{user_total})",
                inline=True,
            )
    ...
    async def inv(self, ctx, userID, subserver_name):
        ...
        if not (sub_role and sub_way_role):
            await ctx.send(
                f"ERROR! Subserver with name {subserver_name} not found.",
                delete_after=3600,
            )
            return
        ...
        if not to_invite:
            await ctx.send(
                f"ERROR! Could not find user with ID {userID}.", delete_after=3600
            )
            return
    ...
    async def create_code(self, ctx, subserver_name):
        ...
        embed = discord.Embed(
            title=f"Invite Code for subserver {subserver_name}",
            description=code,
            color=12008408,
        )
    ...
    async def on_voice_state_update(self, member, _, after):
        ...
        if (
            category
            and category.name == "Subserver Gateway"
            and after.channel
            and after.channel in category.voice_channels
        ):
            # Get Subserver_name.
    ...
    def get_all_subserver_roles(self):
        ...
        re = [
            (sorted_roles[i], sorted_roles[i + 1])
            for i in range(0, len(sorted_roles), 2)
        ]
        return re
        ...
        if to:
            ...
            if sub_role and sub_way_role:
                await member.add_roles(
                    sub_role, reason=f"Member {member.name} switched subserver."
                )
        return True
    ...
    async def update_subserver_info(self):
        ...
        for subserver in subservers:
            ...
            if new_info != current_info:
                await subserver.edit(
                    name=subserver_name + " " + str(new_info).replace(", ", "/")
                )
