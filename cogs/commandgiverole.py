...
class Commandgiverole(commands.Cog):
    ...
    @commands.group(
        name="table", brief="Manage a table to give/remove roles via reactions."
    )
    @isBotModCommand()
    ...
    async def list(self, ctx):
        ...
        for table_name, table_text in self.list_table():
            ...
            embed.add_field(
                name=table_name,
                value=table_text_sanatized[:20]
                + "..." * (len(table_text_sanatized) > 20),
                inline=True,
            )
    ...
    async def rm(self, ctx, table_name):
        ...
        message = await self.get_message(
            ctx, table_content["channelid"], table_content["messageid"]
        )
        if message is None:
            return
        """
    ...
    async def update(self, ctx, table_name):
        ...
        reactions_to_remove = [
            reaction
            for reaction in message.reactions
            if reaction.emoji.name not in table_content["reactions"].keys()
        ]
        ...
        emoji_to_add = [
            emoji
            for emoji in table_content["reactions"].keys()
            if emoji not in present_emojis
        ]
